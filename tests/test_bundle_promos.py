...
def test_2x1_across_two_different_products_frees_the_cheaper_one(client, db):
    ...
    resp = client.post('/api/orders', json={
        'items': [{'id': expensive.id, 'quantity': 1}, {'id': cheap.id, 'quantity': 1}],
...
def test_3x2_frees_cheapest_of_three(client, db):
    ...
    resp = client.post('/api/orders', json={
        'items': [{'id': a.id, 'quantity': 1}, {'id': b.id, 'quantity': 1}, {'id': c.id, 'quantity': 1}],
        'customerName': 'Cliente', 'phone': PHONE,
        'deliveryMode': 'retira', 'paymentMethod': 'efectivo',
    })
    order = Order.query.order_by(Order.id.desc()).first()
...
def test_two_full_groups_both_get_cheapest_free(client, db):
    product = _create_product(db, price=1000, name='Unico')
    _create_promo(db, [product], buy_quantity=2, pay_quantity=1)

    resp = client.post('/api/orders', json={
        'items': [{'id': product.id, 'quantity': 4}],  # 2 complete groups of 2
        'customerName': 'Cliente', 'phone': PHONE,
        'deliveryMode': 'retira', 'paymentMethod': 'efectivo',
    })
    order = Order.query.order_by(Order.id.desc()).first()
...
def test_promo_ignores_products_not_in_its_list(client, db):
    ...
    resp = client.post('/api/orders', json={
        'items': [{'id': eligible.id, 'quantity': 1}, {'id': other.id, 'quantity': 1}],
        'customerName': 'Cliente', 'phone': PHONE,
        'deliveryMode': 'retira', 'paymentMethod': 'efectivo',
    })
    order = Order.query.order_by(Order.id.desc()).first()
...
def test_inactive_promo_does_not_apply(client, db):
